...
static const fast::string DICT[4573] = {
...
size_t verify_map_entries(fast::unordered_map<fast::string, size_t>& map) {
  ...
  std::cout << "Total entries expected: " << DICT_SIZE << '\n';
  return errors;
}

// Function to perform basic operations test
void test_basic_operations(fast::unordered_map<fast::string, size_t>& map) {
  assert(map.count() == DICT_SIZE);
  assert(map.contains("wrongly"));

  map.remove("penis");  // Non-existent word
  assert(map.count() == DICT_SIZE);

  map.remove("wrongly");
  assert(map.count() == DICT_SIZE - 1);

  map["wrongly"] = 17;
  assert(map["wrongly"] == 17);

  map.erase();
  assert(map.count() == 0);
}

// Function to fill map with dictionary words
void fill_map(fast::unordered_map<fast::string, size_t>& map) {
  std::cout << "Filling map with dictionary words...\n";
  for (size_t i = 0; i < DICT_SIZE; ++i) {
    map[DICT[i]] = i;
    if (i % 500 == 0) {
      std::cout << "Progress: " << i << "/" << DICT_SIZE << " entries\n";
    }
  }
  std::cout << "Map filled with " << map.count() << " entries\n";
}

// Function to save map to file
bool save_map(fast::unordered_map<fast::string, size_t>& map, const char* filename) {
...
bool load_map(fast::unordered_map<fast::string, size_t>& map, const char* filename) {
  std::cout << "Loading map from " << filename << "...\n";
  int fd = open(filename, O_RDONLY);
  if (fd < 0) {
    std::cout << "File doesn't exist, cannot load\n";
    return false;
  }

  bool load_success = map.load(fd);
  close(fd);
  std::cout << "Loading " << (load_success ? "successful" : "failed") << "\n";
...
void test_fast_umap() {
  ...
  fast::unordered_map<fast::string, size_t> map;

  // Try to load existing map
  bool file_exists = load_map(map, filename);

  if (file_exists) {
    // File exists - verify entries
    if (verify_map_entries(map) > 0) {
      std::cerr << "ERROR: Map verification failed after loading\n";
      return;
    }

    // Create a copy before testing destructive operations
    fast::unordered_map<fast::string, size_t> map_copy;
    for (size_t i = 0; i < DICT_SIZE; ++i) {
      if (map.contains(DICT[i])) {
        map_copy[DICT[i]] = map[DICT[i]];
      }
    }

    // Test basic operations on the original map
    test_basic_operations(map);

    // Save the loaded map (copy) to ensure persistence works multiple times
    if (!save_map(map_copy, filename)) {
      std::cerr << "ERROR: Failed to save map after loading\n";
      return;
    }
  } else {
    // No existing file - create new map
    ...
    // Verify entries
    if (verify_map_entries(map) > 0) {
      std::cerr << "ERROR: Map verification failed after creation\n";
      return;
    }

    // Save the map
    if (!save_map(map, filename)) {
      std::cerr << "ERROR: Failed to save new map\n";
      return;
    }

    // Load the map again to verify persistence
    fast::unordered_map<fast::string, size_t> loaded_map;
    if (!load_map(loaded_map, filename)) {
      std::cerr << "ERROR: Failed to reload map after saving\n";
      return;
    }

    if (verify_map_entries(loaded_map) > 0) {
      std::cerr << "ERROR: Map verification failed after reloading\n";
      return;
    }

    // Make a copy before destructive testing
    fast::unordered_map<fast::string, size_t> map_for_tests;
    fill_map(map_for_tests);

    // Test basic operations on the copy
    test_basic_operations(map_for_tests);

    // Save the original loaded map again to ensure persistence works multiple times
    if (!save_map(loaded_map, filename)) {
      std::cerr << "ERROR: Failed to save map after verification\n";
      return;
    }
  }
  ...
  std::cout << "fast map time: " << duration << '\n';
}
int main() {
  test_fast_umap();
  return 0;
