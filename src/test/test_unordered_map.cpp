...
void test_fast_umap() {
  ...
  assert(map.count() == 4573);
  assert(map.contains("wrongly"));
  map.remove("penis");
  assert(map.count() == 4573);
  map.remove("wrongly");
  assert(map.count() == 4572);
  map["wrongly"] = 17;
...
void test_std_umap() {
  ...
  for (size_t i = 0; i < DICT_SIZE; ++i) {
    map[std::string(DICT[i].begin())] = i;
  }

  assert(map.size() == 4573);
  assert(map.contains("wrongly"));
  map.erase("penis");
  assert(map.size() == 4573);
  map.erase("wrongly");
  assert(map.size() == 4572);
  map["wrongly"] = 17;
  assert(map["wrongly"] == 17);
  map.clear();
  assert(map.size() == 0);
  ...
  auto end_time = std::chrono::high_resolution_clock::now();
  auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time);
  std::cout << "std map time: " << duration << '\n';
}
