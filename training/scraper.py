...
RESULTS_DIR = "quoted-results"


def search(query: str) -> list[str]:
    """Perform a search on DuckDuckGo and return a list of result URLs."""
    quoted_query = " ".join('"' + word + '"' for word in query.split())
    data = {"q": quoted_query, "kl": "en-us"}
    try:
...
def save_results(query: str, results: list[str]):
    ...
    filename = os.path.join(RESULTS_DIR, f'{" + ".join(query.split())}.txt')
